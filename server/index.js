...
app.post("/signin", (req, res) => {
  ...
  db.query("SELECT customer.email, customer.CustomerID FROM customer WHERE customer.email = (?) AND customer.password = (?)",
  [email, pass],
  (err, result) => {
    if(err)
    {
    ...
    {
      db.query("SELECT staff.email, staff.employee_type, staff.StaffID FROM staff WHERE staff.email = (?) AND staff.password = (?)",
      [email, pass],
      (err, result) => {
        if(err)
        {
...
app.get("/viewinventory", (req, res) => {
  db.query("SELECT inventory_item, category, quantity FROM inventory", (err, result) => {
    if (err) 
    {
      console.log(err);
    }
...
app.post("/firingstaff", (req, res) => {
  ...
  (err, result) => {
    if (result.length != 0)
    { 
      db.query("DELETE FROM staff WHERE email = (?)",
      [email],
      function (err,result)
        {
          if (err)
...
app.post("/additem", (req, res) => {
  ...
  db.query("SELECT inventory_item FROM inventory WHERE inventory_item = (?)",
  [inventory_item],
  (err, result) => {
    if (result.length == 0)
    {
      db.query("INSERT INTO inventory VALUES (?,?,?,?,?)",
        [inventory_item, staffid, category, quantity, date]
      )
      res.send("success");
    }
    else
...
app.post("/changequantity", (req, res) => {
  ...
  (err, result) => {
    if (result.length != 0)
    {
      db.query("UPDATE inventory SET quantity = (?) WHERE inventory_item = (?)",
...
app.get("/viewrooms", (req, res) => {
  db.query("SELECT room_number, type_of_room, status FROM hotel_rooms WHERE status = 'Available'", (err, result) => {
...
app.post("/cancelreservations", (req, res) => {
  ...
  db.query("SELECT bookingID, room_number FROM reservations WHERE bookingID = (?) and CustomerID = (?)",
  [booking_id, cust_id],
  (err, result) => {
    if (result.length != 0)
    {
      db.query("UPDATE hotel_rooms SET status = 'Available' WHERE room_number = (?)",
...
app.post("/supdateaccinfo", (req, res) => {
  ...
  db.query("UPDATE staff SET First_Name = (?), Last_Name = (?) WHERE StaffID = (?)" , [fname, lname, staff_id] );
  res.send("success");
...
app.post("/cupdateaccinfo", (req, res) => {
  ...
  db.query("UPDATE customer SET First_Name = (?), Last_Name = (?) WHERE CustomerID = (?)" , [fname, lname, cust_id] );
  res.send("success");
...
app.post("/setroomrate", (req, res) => {
  ...
  db.query("SELECT * FROM room_rates WHERE Type_of_room = (?)",
  [roomtype],
  (err, result) => {
    if (result.length != 0) // room type exists
    { 
      db.query("UPDATE room_rates SET Room_rate = (?) WHERE Type_of_room = (?)" , [newrate, roomtype]);
      res.send("success");
    }
    else
    {
...
app.post("/makepayment", (req, res) => {
  ...
  db.query("SELECT payments_due FROM customer WHERE CustomerID = (?)",
    [custid], (err, result) => {
      if(result[0].payments_due != '0')
      {
        db.query("UPDATE customer SET payments_due = (?) WHERE CustomerID = (?)", [result[0].payments_due - payment, custid]);
        res.send("success")
      }
      else
      {
...
app.post("/deleteroom", (req, res) => {
  ...
  db.query("SELECT * FROM hotel_rooms WHERE room_number = (?)",
  [roomnum],
  (err, result) => {
    if (result.length != 0)
    {
      db.query("SELECT room_number FROM reservations WHERE room_number = (?)", [roomnum], (err, result2) =>{
        if(result2.length == 0)
        {
          db.query("DELETE FROM hotel_rooms WHERE room_number = (?)", [roomnum]);
          res.send("success");
        }
        else
        {
...
app.get("/viewstaff", (req, res) => {
  db.query("SELECT staffID, first_name, last_name, employee_type, salary, shift_type FROM staff", (err, result) => {
    if (err) 
    {
      console.log(err);
    }
...
app.post("/addfacility", (req, res) => {
  ...
  db.query("SELECT * FROM facilities WHERE name_of_facility = (?)",
  [name],
  (err, result) => {
    if (result.length == 0)
    {
      db.query("SELECT * FROM facilities", function(err, number)
      {
        ...
        {
          db.query("SELECT MAX(FacilityID) as id FROM facilities", function (err,facility_id)
          {
              db.query("INSERT INTO facilities VALUES (?,?,?)",
                [parseInt(facility_id[0].id) + 1, name, rate]
              );
          });
        }
      });
...
app.post("/deletefacility", (req, res) => {
  ...
  (err, result) => {
    if (result.length != 0)
    { 
      db.query("DELETE FROM facilities WHERE name_of_facility = (?)",
      [name],
      function (err,result)
        {
          if (err)
...
app.post("/availfacility", (req, res) => {
  ...
  db.query("SELECT BookingID, room_number FROM reservations WHERE customerID = (?)",
  [custID],
  (err, result) => {
    ...
    {
      db.query("SELECT facilityID from facilities WHERE Name_of_facility = (?)",
      [name],
      function (err,result2){
        if (result2.length != 0)
        {
          db.query("INSERT INTO facilities_availed VALUES (?,?,?,?,?)", 
          [result[0].BookingID, result[0].room_number, date, result2[0].facilityID, quantity]);
          res.send("success");
        }
        else
        {
...
app.post("/setstaffsch", (req, res) => {
  ...
  (err, result) => {
    if (result.length != 0)
    { 
      db.query("UPDATE staff SET Shift_type = (?) WHERE email = (?)", [sch, email]);
      res.send("success");
    }
    else
    {
...
app.post("/staffcancelreservations", (req, res) => {
  ...
  db.query("SELECT bookingID, room_number FROM reservations WHERE bookingID = (?)",
  [booking_id],
  (err, result) => {
    if (result.length != 0)
    {
...
app.get("/bookings", (req, res) => {
  ...
  db.query("SELECT bookingID, SUBSTRING(check_in_date, 1, 10) AS check_in_date, SUBSTRING(check_out_date, 1, 10) AS check_out_date, customerID FROM reservations",
    (err, result) => {
    if (err) 
    {
      console.log(err);
...
app.get("/viewfacilities", (req, res) => {
  db.query("SELECT facilityID, name_of_facility, facility_rate FROM facilities", (err, result) => {
    if (err) 
    {
      console.log(err);
    }
...
app.post("/cupdatepass", (req, res) => {
  ...
  db.query("UPDATE customer SET password = (?) WHERE CustomerID = (?)" , [password, custid]);
  res.send("success");
});
...
app.post("/supdatepass", (req, res) => {
  ...
  db.query("UPDATE staff SET password = (?) WHERE StaffID = (?)" , [password, staffid]);
  res.send("success");
});
...
app.post("/updatereservation", (req, res) => {
  ...
  db.query("SELECT * FROM reservations WHERE BookingID = (?) and CustomerID = (?)" , [book, custid], (err, result) => {
    if(result.length == 0)
    {
      res.send("No booking found")
    }
    ...
    {
      db.query("UPDATE reservations SET check_in_date = (?), check_out_date= (?) WHERE BookingID = (?) and CustomerID = (?)" , [checkin, checkout, book, custid]);
      res.send("success");
    }
  });
});
...
app.get("/viewbookingid", (req, res) => {
  db.query("SELECT bookingID, SUBSTRING(check_in_date, 1, 10) AS check_in_date, SUBSTRING(check_out_date, 1, 10) AS check_out_date FROM reservations", (err, result) => {
    if (err) 
    {
      console.log(err);
    }
