...
app.post("/auth", function (request, response) {
  ...
  if (username && password) {
    ...
    connection.query(
      "SELECT * FROM hotel_management.accounts WHERE username = ? AND password = ?",
      [username, password],
...
router.post("/SignUp", async (request, response) => {
  ...
  var paydue = "0";
  sql_statement =
    "INSERT INTO Customer (CustomerID, Email, FirstName, LastName, Password, Payments_Due) VALUES (?,?,?,?,?,?)";
  connection.query(
    sql_statement,
    [, email, fname, lname, password, paydue],
    (err, rows) => {
      if (err) {
        console.log("ERROR", err);
...
router.post("/HireStaff", async (request, response) => {
  ...
  var paydue = "0";
  sql_statement =
    "INSERT INTO Staff (StaffID, Email, FirstName, LastName, Password,Employee_Type,Salary, Shift_type ) VALUES (?,?,?,?,?,?,?,?)";
  connection.query(
    sql_statement,
    [, email, fname, lname, password, Employee, Salary, Shift],
    (err, rows) => {
      if (err) {
        console.log("ERROR", err);
        response.sendStatus(400);
...
router.post("/UpdateInventory", async (request, response) => {
  ...
  sql_statement =
    "INSERT INTO Inventory (Inventory_item, StaffID, Category, Quantity, Date) VALUES (?,?,?,?,?)";
  connection.query(
    sql_statement,
    [invName, current_loggedin_id, category, quantity, date],
    (err, rows) => {
      if (err) {
        // console.log("ERROR",err)
        // response.sendStatus(400)
